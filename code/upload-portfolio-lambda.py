...
def lambda_handler(event, context):
    ...
    try:
        ...
        if len(the_zipfile) > 0:
            ...
            topic.publish(Subject="SUCCESS-Michael Ryan AWS Portfolio", Message="Portfolio published successfully!")
        else:
            print ("No Zip file found!")
            topic.publish(Subject="FAILED-Michael Ryan AWS Portfolio", Message="Portfolio failed to publish! No Zip file found.")
            raise
    except:
        topic.publish(Subject="FAILED-Michael Ryan AWS Portfolio", Message="Portfolio failed to publish! Process failure.")
        raise
